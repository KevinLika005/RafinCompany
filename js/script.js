...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function syncActiveItem(navbar) {
    ...
    function findLinkByPath(targetPath) {
      ...
      navbar.querySelectorAll(".rd-navbar-nav a[href]").forEach(function (link) {
        ...
        if (!url.hash && normalizePath(url.pathname) === normalizedTargetPath) {
          fallbackMatch = link;
        }
      });
    ...
    if (!matchedLink && relatedPathFallbacks[currentPath]) {
      matchedLink = findLinkByPath(relatedPathFallbacks[currentPath]);
    }
