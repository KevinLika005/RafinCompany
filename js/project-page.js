document.addEventListener('DOMContentLoaded', () => {
  ...
  const preloadHeroMedia = (mediaItem) => {
    if (!document.head || !mediaItem) return;
  ...
  const showMissingState = () => {
    ...
    if (window.SeoHead && typeof window.SeoHead.applyRouteMeta === 'function') {
      window.SeoHead.applyRouteMeta({
        pageType: 'project',
        titleOverride: `${defaultMissingTitle} | Rafin Company`,
        descriptionOverride: defaultMissingDesc,
        indexable: false
      });
    }
  };
  ...
  if (allProjectsLink) {
    allProjectsLink.href = buildUrl('projects.html?lang=en');
  ...
  const populateNavCard = (card, targetProject, navLabelKey) => {
    ...
    card.href = buildUrl(`project.html?slug=${encodeURIComponent(targetProject.slug)}&lang=en`);
